...
session_start();
	...
	if(!empty($s_name) && !empty($s_enroll_no) && !empty($s_department) && !empty($s_year) && !empty($s_semester) && !empty($s_email_id) && !is_numeric($s_name))
	{
		...
		//save to database
		$rid = random_num(20);
		$query = "insert into results4_1 (rid,s_name,s_enroll_no,s_department,s_year,s_semester,s_email_id,GPA) values ('$rid','$s_name','$s_enroll_no','$s_department','$s_year','$s_semester','$s_email_id','$GPA')";
